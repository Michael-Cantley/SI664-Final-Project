...
-- MIKE WRITE THIS SQL SELECT TO POPULATE game TABLE COLUMNS INCLUDING FK LOOKUP VALUES
-- REQUIRES JOINING ON LOOKUP TABLE *_name columns (string to string)
-- FROM x
-- LEFT JOIN y ON TRIM(x.string) = TRIM(y.string)
-- ...
SELECT tg.game_name, p.platform_id, tg.year_released, g.genre_id, pub.publisher_id,
       tg.north_america_sales, tg.europe_sales, tg.japan_sales, tg.other_sales,
       tg.global_sales, tg.critic_score, tg.critic_count, tg.user_score,
       tg.user_count, tg.developer_name, r.rating_id
FROM temp_game tg
     LEFT JOIN platform p
               ON TRIM(tg.platform_name) = TRIM(p.platform_name)
     LEFT JOIN genre g
